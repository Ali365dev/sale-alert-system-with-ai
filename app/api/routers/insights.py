"""AI Insights endpoints — daily digest, best offers, expiring soon,
recommended actions. Ported from api/insights.py — logic and response shape
unchanged. call_llm() is a blocking network call; this stays a plain `def`
route so FastAPI/Starlette runs it in its worker thread pool rather than
blocking the event loop (see migration plan's async strategy decision)."""
import json
