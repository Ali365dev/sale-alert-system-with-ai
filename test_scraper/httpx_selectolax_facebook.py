...
def main() -> int:
    ...
    try:
        ...
        return emit("httpx + selectolax", "Facebook", URL, time.perf_counter() - started, status="FAILED", reason=str(exc))
    ...
    try:
        ...
        if response.status_code >= 400:
            return emit("httpx + selectolax", "Facebook", URL, elapsed, status="FAILED", reason=f"HTTP {response.status_code}")
        if not is_useful(name, post_urls, texts, images):
            return emit(
                "httpx + selectolax", "Facebook", URL, elapsed, status="FAILED",
                reason=f"HTTP {response.status_code}; title={name!r}; no posts/images in static HTML (likely a JS shell or login wall)",
            )
        return emit("httpx + selectolax", "Facebook", URL, elapsed, status="SUCCESS", profile_name=name, post_urls=post_urls, texts=texts, images=images)
    except Exception as exc:
        return emit("httpx + selectolax", "Facebook", URL, time.perf_counter() - started, status="FAILED", reason=f"{type(exc).__name__}: {exc}")
