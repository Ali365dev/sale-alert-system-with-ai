...
def main() -> int:
    ...
    try:
        ...
        return emit("Camoufox", "Instagram", URL, time.perf_counter() - started, status="FAILED", reason=str(exc))
    ...
    try:
        ...
        if not is_useful(name, post_urls, texts, images):
            return emit("Camoufox", "Instagram", URL, elapsed, status="FAILED", reason=f"Loaded page title={name!r} but no public posts/images extracted")
        return emit("Camoufox", "Instagram", URL, elapsed, status="SUCCESS", profile_name=name, post_urls=post_urls, texts=texts, images=images)
    except Exception as exc:
        return emit("Camoufox", "Instagram", URL, time.perf_counter() - started, status="FAILED", reason=f"{type(exc).__name__}: {exc}")
